...
B=Path(__file__).resolve().parent;O=B/'extended'/'models';O.mkdir(exist_ok=True)
f=pd.read_csv(B/'extended'/'features_180.csv')
cols=list(f.columns[4:]);people=sorted(f.pid.unique())
core=['peak','area_60_120','area_centroid']
original=cols[:12]
context=['baseline','pre_slope','pre_range','pre_recent_slope']
timing=[v for v in cols[12:] if v not in context]
groups=[original,original+timing,original+context,cols]
candidates=[(g,degree,alpha) for g in groups for degree in [1,2] for alpha in [1.,10.,100.]]
plan={'candidates':24,'features':groups,'degree':[1,2],'ridge_alpha':[1,10,100],
 ...
 'context':'premeal context enters as absolute values standardized on training rows only',
...
def data(train,test):
 ...
 def one(p):
  ...
  out=(qry[p]-cal[p].mean(axis=0))/den
  for k,col in enumerate(cols):
   if col in context:out[:,k]=qry[p][:,k]
  return out
...
out=[];alternatives=[];selection=[];inner_log=[]
for held in people:
 ...
 inner_log.extend(dict(pid=int(held),candidate=k,inner_hits=-v[0],inner_mae=v[1]) for k,v in enumerate(scores))
...
pd.DataFrame(inner_log).to_csv(O/'inner_scores.csv',index=False)
...
(O/'manifest.json').write_text(json.dumps({str(x):hashlib.sha256(x.read_bytes()).hexdigest() for x in [Path(__file__),B/'extended'/'features_180.csv',O/'PLAN.json']},indent=2))
